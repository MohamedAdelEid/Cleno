  ...
  >
    <div className="flex flex-1 flex-col gap-2.5 sm:flex-row sm:items-center">
      ...
      <SearchableSelect
        ...
        searchPlaceholder={labels.filterCustomer}
    ...
    <ViewSwitcher
      ...
      labels={{ list: labels.viewList, board: labels.viewBoard }}
    />
  </motion.div>
)
...
const ViewSwitcher = ({ viewMode, onViewModeChange, labels }: ViewSwitcherProps) => {
  const modes = [
    { id: 'list' as const, icon: List, label: labels.list },
    { id: 'board' as const, icon: LayoutGrid, label: labels.board },
  ]
