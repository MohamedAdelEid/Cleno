...
import type { OperationalBagStats } from '@/domain/entities'
...
import { statSparklineTrends } from '@/presentation/components/admin/operational-bags/operational-bags.data'
...
const OverviewSkeletonCard = ({ index }: { index: number }) => (
  <motion.div {...fadeUp(index * 0.05)} className="overflow-hidden rounded-xl border border-border/80 bg-card">
...
const StatGroup = ({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={cn('grid gap-3', keys.length === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-2 xl:grid-cols-4')}>
        {keys.map((key, index) => {
          ...
          return (
            <DashboardStatCard
              ...
              icon={statIcons[key]}
              sparklineData={statSparklineTrends[key] ?? []}
              sparklineTrend={statSparklineTrend[key]}
              highlight={key === 'missingBags' && stats.missingBags > 0 ? String(stats.missingBags) : undefined}
            />
          )
        })}
      </div>
...
export const BagsOverviewSection = ({
  ...
  return (
    <div className={cn('space-y-6', className)}>
      <StatGroup title={t('statsInventory')} keys={inventoryStats} stats={stats} startIndex={0} />
      <StatGroup title={t('statsOperations')} keys={operationsStats} stats={stats} startIndex={3} />
    </div>
  )
}
